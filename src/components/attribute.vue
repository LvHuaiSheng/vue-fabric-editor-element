<template>
  <div class="box" v-if="mSelectMode === 'one'">
    ...
    <div v-show="textType.includes(mSelectOneType)">
      ...
      <div class="flex-view">
        <div class="flex-item">
          ...
          >
            <el-radio-button v-for="(item, i) in textAlignList" :label="item" :key="item">
    ...
    <div v-show="baseType.includes(mSelectOneType)">
      ...
      <!-- 多边形边数 -->
      <el-row v-if="mSelectOneType === 'polygon'" :gutter="12">
        <el-col :span="6">
          <InputNumber
      ...
      <el-row>
        <el-col :span="24">
          <div class="ivu-col__box flex-view">
    ...
    <!-- ID属性 -->
    <div>
      <el-divider content-position="left">{{ $t('attributes.id') }}</el-divider>
      <div class="flex-view">
        <div class="flex-item">
          <span class="label">{{ $t('attributes.id') }}</span>
          <div class="content slider-box">
            <el-input v-model="baseAttr.id" @change="changeCommon('id', baseAttr.id)"></el-input>
          </div>
        </div>
      </div>
    </div>
